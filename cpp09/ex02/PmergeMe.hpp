...
class PmergeMe {
	...
	public:
		...
		std::vector<int> merge_recursion(std::vector<std::pair<int, int> >& pairs, int &level);
		std::deque<int> merge_recursion_deque(std::deque<std::pair<int, int> >& pairs, int &i);
		...
		void printDequeOfPairs(std::deque< std::pair<int, int > > & deque);
